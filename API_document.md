...
## 共通事項
  ...
  - ベースURL: `http://192.168.35.242:8000`
...
#### ユーザの特定
  ...
  - エンドポイント: POST /reset/verify
  - 説明: パスワードを忘れたときの小学校情報を使ったユーザ特定
  - ユーザ認証: いらない
  ...
  - レスポンス
    - 成功時
      {
        "message": "ユーザの確認が成功しました．"
      }
      - 成功時は後述のパスワードリセットAPIを叩く
...
### 論文検索
  ...
  - リクエスト
    - Content-Type: application/json
      {
        "question": "string",
        "lang": "string",  # これいらないので，将来的には消す予定です
        "category: "string"
      }
      - question: プロンプト
      - category: 研究テーマ名
  ...
  - レスポンス
    ...
    - 失敗時（データベース中に存在しない論文がヒットした時）
      ステータスコード: 404
      {
        "detail": "論文{paper_id}が見つかりませんでした．"
      }
      - エラー発生時はエラーメッセージを表示（？）
...
### 個別論文への質問
  ...
  - リクエスト
    - Content-Type: application/json
      {
        "question": "string",
        "language": "string"  # これいらないので，将来的には消す予定です
        "category: "string"
      }
      - question: プロンプト
      - category: 研究テーマ名
  ...
  - レスポンス
    - 成功時
      ...
      - 成功時は質問への回答がチャット画面下に表示される

    - 失敗時（データベース中に存在しない論文を探索するとき）
      ステータスコード: 404
      {
        "detail": "論文{paper_id}が見つかりませんでした．"
      }
